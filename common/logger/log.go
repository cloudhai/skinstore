...
func init(){
	...
	if err != nil && os.IsNotExist(err){
		...
		if err != nil{
			slog.Fatal("mkdir Logs folder failed")
		}
	}
	...
	l = &stclog{c:make(chan string,MAXSIZE),level:getLevel("info")}
	rotateLog(func() {
		logfile.Close()
		err = os.Rename(logPath+logName,logPath+prefix+time.Now().Format("2006-01-02")+".log")
		if err != nil {
			slog.Println(err)
		}
		logfile,err = os.OpenFile(logPath+logName,os.O_APPEND|os.O_CREATE,os.ModePerm)
